//params09_exer2_ans1.c
//Exercise on Passing an array of C structures (or records) 
//as parameter to functions

//Answer to params09_exer2.c
...
char * getHighest(node *a, int n){ //should return the name of the student
									 // with the highest grade out of the  
									 // n entries from "array a in main"
	int i, maxindex = 0;
	
	for(i=0;i<n;i++)
		if(a[i].grade<a[maxindex].grade)
			maxindex=i;
...
int main(){
	...
	//Exercise: call getHighest to get name of the student 
