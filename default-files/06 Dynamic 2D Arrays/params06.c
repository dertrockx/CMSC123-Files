...
void input(int **a, int m, int n){ 
	//"dynamic 2D array is only passed as a pointer to a pointer
	int i,j;
...
//void output(int a[][3], int m, int n){ //INVALID, see comments in input
...
int main(){
	...
	input(a, m, n); // dynamic 2D array is also passed as pointer
	output(a, m, n);// but, they are received differently (see formal parameter notes)
